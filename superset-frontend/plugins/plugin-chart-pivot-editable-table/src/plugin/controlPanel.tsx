    ...
    },
    {
      ...
      controlSetRows: [
        [
          {
            ...
            config: {
              type: 'TextControl',
              label: t('Backend API URL'),
              description: t('URL for the backend API to send edit payloads.'),
              placeholder: 'http://localhost:8080/api/v1/data',
              renderTrigger: true,
            },
          },
        ],
        [
          ...
          //     description: t('Select which metrics can be edited inline.'),
          //     multi: true,
          //     freeForm: true,
          //     default: [],
          //     mapStateToProps: (state: ControlPanelState) => {
          //       const datasource = state?.datasource as any;
          //       const allMetrics = ensureIsArray(datasource?.metrics);
          //       const verboseMap = datasource?.hasOwnProperty('verbose_map')
          //         ? datasource?.verbose_map
          //         : datasource?.columns || {};
          //       const metricOptions = allMetrics.map((value: any) => {
          //         if (typeof value === 'string') {
          //           return {
          //             value,
          //             label: Array.isArray(verboseMap)
          //               ? value
          //               : verboseMap[value]?.verbose_name || value,
          //             column_name: value,
          //           };
          //         }
          //         return {
          //           value: value.column?.column_name || value.label,
          //           label: value.label,
          //           column_name: value.column?.column_name || value.label,
          //         };
          //       });
          //       return {
          ...
          //       };
          //     },
          //     rerender: ['metrics', 'datasource_id'], // Re-render when metrics or datasource change
          //   },
          // },

          {
            name: 'editableMetrics',
            config: {
              type: 'SelectControl', // Changed from SelectAsyncControl to SelectControl
              label: t('Editable Metrics'),
              description: t('Select which metrics can be edited inline.'),
              multi: true,
              freeForm: false, // Changed from true to false for better control
              default: [],
              mapStateToProps: (state: ControlPanelState) => {
                const allAvailableMetrics = ensureIsArray(
                  state?.controls?.metrics?.value,
                );
                const currentEditableMetrics = ensureIsArray(
                  state?.controls?.editableMetrics?.value,
                );

                const options = allAvailableMetrics
                  .map((metric: any) => {
                    if (!metric) {
                      return null;
                    }
                    ...
                    if (typeof metric === 'string') {
                      ...
                      columnName = metric.column?.column_name || metric.label;
                    } else if (
                      ...
                      // Fallback for complex metrics or unexpected structures
                      const fallbackValue =
                        metric.column?.column_name ||
                        metric.label ||
                        JSON.stringify(metric);
                      metricValue = fallbackValue; // Assign metricValue first
                ...
                const validSelections = currentEditableMetrics.filter(
                  (selected: string) =>
                    options.some(
